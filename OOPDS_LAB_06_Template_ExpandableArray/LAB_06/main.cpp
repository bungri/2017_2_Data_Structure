...
Algorithm merge(T_ExpandableArray &eA)
{
	if (this->capacity > (this->num + eA.num)) then
		reserve((this->num + eA.num)
	for i ก็ 0 to (this->num + eA.num) do
		insert(this->num + i, eA.t_array[i])
	this->quicksort()
	return this->num
}
