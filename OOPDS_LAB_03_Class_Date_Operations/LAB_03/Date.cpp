...
{
	output << "Date [" << setw(4) << date.getYear() << "." << setw(2) << date.getMonth() << "." << setw(2) << date.getDay() << ", " << date.getName() << "(" << weekDayList[date.getWeekDay()] << ")]" << endl;
	return output;
}
...
Date::Date(int y, int m, int d, string n) // constructor
{
	setDate(y, m, d);
	setName(n);
}

 Date::Date(int y, int m, int d, char n[]) // constructor
{
	 setDate(y, m, d);
	 setName(n);
}
...
const Date Date::operator+(int dd)
{
	this->day += dd;
	int temp = 0;
	while (day > monthlyDays[this->month-1])
	{
...
const Date Date::operator-(int dd)
{
	...
	while (this->day < 1)
	{
		...
		if ((isLeapYear(this->year)) && (this->month - 1 == 2))
			temp += 29;
		else
			temp += monthlyDays[this->month - 1];
		this->day += temp;
